    ...
    return (
        <div>
            <div key={cmt.id} className="comment-card">
                <div className="comment-card-name-comment">{cmt.date}</div>
                ...
                {cmt.status ? <i onClick={() => handleDelete(cmt)} style={{marginLeft:'20px', cursor: 'pointer'}} className="fas fa-trash"></i> : ''}
                {cmt.status ? <i onClick={() => handleEdit(cmt)} style={{marginLeft:'20px', cursor: 'pointer'}} className="fas fa-edit"></i> : ''}
            </div>
        </div>
    )
}
